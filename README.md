...
- **计划 vs 实际**  
  右键色条打开完成对话框，在迷你日轴上圈出实际用时。完成后：计划条置灰，实际条按色卡叠在上面。未完成且已逾期的计划条同样置灰。可再次右键实际条取消完成。

- **标签与过滤**  
  任务可挂多个标签，其中一个是主标签（决定基础色）。顶栏菜单按标签过滤；命中主标签或任一附属标签即显示。

- **色卡**  
  设置里维护色卡（ARGB / HSL），可指定默认色、删除时改绑到另一张。主题主色跟随默认色卡。任务也可单独覆盖色卡。

- **本机优先**  
  无账号、无云同步、无遥测。数据库打不开时会提示备份损坏文件后再重建，不会静默清空。

- **备份**  
  设置页导出 / 导入 JSON（当前备份格式 version 2），包含任务、标签、色卡与设置。导入时校验结构，重复任务会跳过。
...
| 单击色条 | 右侧抽屉打开任务表单（标题、计划时间、标签、色卡、备注） |
| 右键色条 | 完成对话框：圈选实际起止；已完成任务可右****键实际条取消完成 |
| 顶栏标签菜单 | 按标签过滤甘特条 |
| 设置（侧栏） | 可视时段（默认 8:00–22:00）、色卡、标签、导出 / 导入 |
...
- **设置 → 导出**：写出 `gantday-backup.json`（任务、标签、色卡、可视时段等）
...
lib/
  ...
  app.dart                  # 服务注入与主题（跟随默认色卡）
  domain/                   # 纯逻辑：任务/标签/色卡、甘特几何、完成轴
